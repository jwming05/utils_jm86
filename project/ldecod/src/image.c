...
{
  int current_header;
  Slice *currSlice = img->currentSlice;		//当前条带指针
  ...
  img->newframe = 1;
	...
	//循环中就没改过currSlice->next_header的值，只有在current_header==EOS时，序列结束，才会退出编码
  while ((currSlice->next_header != EOS && currSlice->next_header != SOP))
  {
    current_header = read_new_slice();		//读取新的数据
...
//  int i;
  ...
  while (1)
  {
    ftell_position = ftell(bits);				//得到流式文件的当前读写位置,其返回值是当前读写位置偏离文件头部的字节数(其实就是已经读出的字节数)

    if (input->FileFormat == PAR_OF_ANNEXB)
      ret=GetAnnexbNALU (nalu);					//ret是读取出来的nalu字节数
    else
      ret=GetRTPNALU (nalu);
...
//      printf ("read_new_slice: returning %s\n", "EOS");
    ...
    switch (nalu->nal_unit_type)
    {
      ...
      case NALU_TYPE_IDR:
        ...
        FmoInit (active_pps, active_sps);			//片组映射初始化(初始化了 宏块到片组的映射MbToSliceGroupMap)

        if(is_new_picture())						//是否当前的条带数据为新的图片数据
        {
          init_picture(img, input);					//初始化新帧的一些参数初始化，包括新帧的poc计算
          
          current_header = SOP;						//当前数据为新的图片数据
          //check zero_byte if it is also the first NAL unit in the access unit
          CheckZeroByteVCL(nalu, &ret);
        }
        else
          current_header = SOS;						//当前数据并非新的图片数据，而只是新的条带数据
  
		//初始化参考帧列表, 并重排序（事实上，重排序功能并没使能）
		//将解码器缓冲区dpb中的数据，和参考帧列表list0 list1联立
		/*具体的：
			* 	  这里面，将list初始化，具体的操作有：
...
void init_picture(struct img_par *img, struct inp_par *inp)
{
  ...
  img->pre_frame_num = img->frame_num;			//上一帧的frame_num, 计算当前帧poc时需要
  img->num_dec_mb = 0;
...
void decode_one_slice(struct img_par *img,struct inp_par *inp)
{
  ...
  fprintf(p_trace,"\n*********** POC: %i (I/P) MB: %i Slice: %i Type %d **********\n", img->ThisPOC, img->current_mb_nr, img->current_slice_nr, img->type);
    ...
    // 初始化当前宏块的一些参数
	/*
		1.当前宏块相关坐标 img->mb_x/y（宏块为单位），img->block_x/y(4x4块为单位), img->pic_x/y（像素为单位），除此之外，还有色度块的坐标
		2.当前宏块的条带地址 currMB->slice_nr, dec_picture->slice_id[img->mb_x][img->mb_y]
		3.量化参数，cbp，宏块类型，帧内预测模式，运动矢量残差，预测误差，环路滤波相关参数重置为初始值
	*/
    start_macroblock(img,inp, img->current_mb_nr);
    
//解码宏块**************************************************************************************************************************************
    /* Get the syntax elements from the NAL, 从NAL数据中，获得当前块的信息包括
		1).宏块类型currMB->mb_type(指定帧内4x4(没有具体指定) 帧内16x16各种 帧间)
	*/
