...
int intrapred(
  ...
  int ioff,             //!< pixel offset X within MB					当前4x4块在宏块中x方向上的位置，像素单位
  int joff,             //!< pixel offset Y within MB					当前4x4块在宏块中y方向上的位置，像素单位
  int img_block_x,      //!< location of block X, multiples of 4		当前4x4块在图像中x方向上的位置，4x4块为单位
  int img_block_y)      //!< location of block Y, multiples of 4		当前4x4块在图像中y方向上的位置，4x4块为单位
{
  int i,j;
  int s0;
  int img_y,img_x;
  ...
  byte predmode = img->ipredmode[img_block_x][img_block_y];			//当前宏块的帧内4x4预测模式(已经由传输的数据，转换为正儿八经的预测模式了)

  img_x=img_block_x*4;												//当前4x4块在图像中的位置，像素单位
  img_y=img_block_y*4;												//当前4x4块在图像中的位置，像素单位
