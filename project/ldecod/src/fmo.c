...
int FmoInit(pic_parameter_set_rbsp_t* pps, seq_parameter_set_rbsp_t* sps)
{
  ...
  FmoGenerateMapUnitToSliceGroupMap(pps, sps);				//生成mapUnitToSliceGroup（映射单元到片组的映射表）
  FmoGenerateMbToSliceGroupMap(pps, sps);					//生成MbToSliceGroupMap（宏块到片组的映射表）

  NumberOfSliceGroups = pps->num_slice_groups_minus1+1;		//片组个数
