...
void init_ref_pic_list_reordering()
{
  ...
  currSlice->ref_pic_list_reordering_flag_l0 = 0;		//list0重排序标志
  currSlice->ref_pic_list_reordering_flag_l1 = 0;		//list1重排序标志
}
...
int start_slice()
{
  ...
  Slice *currSlice = img->currentSlice;			//currSlice和img->currentSlice连接，该函数中主要是对currSlice进行操作
  ...
  init_ref_pic_list_reordering();	//初始化参考帧列表重排序, 默认关闭重排序, jm平台中也没有重排序的相关操作
  ...
  for (i=0; i<NumberOfPartitions; i++)
  {
    ...
    if (i==0)     // First partition
      header_len += SliceHeader (0);				//写SH, 并返回片头长度
...
int encode_one_slice (int SliceGroupId, Picture *pic)
{
  ...
  set_ref_pic_num();														//设置ref_pic_num

  if (img->type == B_SLICE)
      compute_collocated(Co_located, listX);

  CurrentMbAddr = FmoGetFirstMacroblockInSlice (SliceGroupId);				//获得当前片的第一个宏块地址
...
{
  int i;
  Picture *currPic = img->currentPicture;			//当前图片
  Slice *curr_slice;								//当前片
  DataPartition *dataPart;							//数据分区
  Bitstream *currStream;							//bit流
...
void set_ref_pic_num()
{
  ...
  //对所有enc_picture->ref_picnum_num[list0/1][list_idx]
