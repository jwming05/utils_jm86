...
FastIntegerPelBlockMotionSearch  (pel_t**   orig_pic,     // <--  not used						当前搜寻块的像素
                  int       ref,          // <--  reference frame (0... or -1 (backward))		参考帧
                  int       list,
                  int       pic_pix_x,    // <--  absolute x-coordinate of regarded AxB block	当前搜索块在图像中的坐标
                  int       pic_pix_y,    // <--  absolute y-coordinate of regarded AxB block	
                  int       blocktype,    // <--  block type (1-16x16 ... 7-4x4)				搜索块类型
                  int       pred_mv_x,    // <--  motion vector predictor (x) in sub-pel units	预测向量（亚像素单位）
                  int       pred_mv_y,    // <--  motion vector predictor (y) in sub-pel units
                  int*      mv_x,         //  --> motion vector (x) - in pel units				运动矢量
                  int*      mv_y,         //  --> motion vector (y) - in pel units
                  int       search_range, // <--  1-d search range in pel units                 运动搜索范围
                  int       min_mcost,    // <--  minimum motion cost (cost for center or huge value)
                  double    lambda)       // <--  lagrangian parameter for determining motion cost
{
  ...
  cand_x = center_x ;
  cand_y = center_y ;
  //Mv_COST是运动矢量代价 但是它不含前面的SAD 只有后面部分
  mcost = MV_COST (lambda_factor, mvshift, cand_x, cand_y, pred_x, pred_y);
  //PartCalMad应该是用于计算SAD的 里面的byte_abs存储绝对值 应该是用于计算SAD部分
