 ...
 *    RefreshPattern[MB]指定了MB是否进行帧内预测
 *    RefreshPattern[MB] == MB, 则该宏块进行帧内预测
 *	  该函数的目的就是生成随机数
 ************************************************************************
 */
...
void RandomIntraInit(int xsize, int ysize, int refresh)
{
  ...
  RefreshPattern = malloc (sizeof (int) * NumberOfMBs);				//对RefreshPattern申请内存
  ...
  for (i= 0; i<NumberOfMBs; i++)
   ...
   for (i=0; i<NumberOfMBs; i++)
   {
     ...
     RefreshPattern [pos] = i;										//对RefreshPattern[pos]进行赋值
   }
}
...
void RandomIntraNewPicture ()
{
  ...
  for (j=0,i=WalkAround; j<NumberIntraPerPicture; j++, i++)
    IntraMBs[j] = RefreshPattern [i%NumberOfMBs];						//IntraMBs[j], 第j个帧内预测宏块地址
}
