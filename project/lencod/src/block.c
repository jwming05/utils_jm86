 ...
 ************************************************************************
 */
//pic_pix_x,			当前子块在图像中的位置，像素单位
//pic_pix_y, 
//&left_available,		返回左方块是否使能
//&up_available,		返回上方块是否使能
//&all_available		返回左方以及上方是否共同使能
// 该函数将预测值保存在img->mprr[mode][x][y]中
void intrapred_luma(int img_x,int img_y, int *left_available, int *up_available, int *all_available)
{
  ...
  // For MB level frame/field coding tools -- set default to imgY
  byte **imgY = enc_picture->imgY;				//编码图像像素，该处用于读出已经编码的像素值，用作预测

  //当前4x4子块在宏块中的位置
  int ioff = (img_x & 15);		//a&0xFF == a%15
  int joff = (img_y & 15);	
  ...
  {
    block_available_left     = pix_a[0].available;			//左方是否使能,a[0]便可判断左方所有子块是否使能
    block_available_up       = pix_b.available;				//上方是否使能
    block_available_up_right = pix_c.available;				//右上是否使能
    block_available_up_left  = pix_d.available;				//左上是否使能
  }
  ...
  *all_available  = block_available_up && block_available_left && block_available_up_left;
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // 2). make horiz and vert prediction
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void intrapred_luma_16x16()
{
  ...
  // imgY_pred 是用来预测的像素
  byte   **imgY_pred = enc_picture->imgY;  // For Mb level field/frame coding tools -- default to frame pred 用邻块重建像素进行预测
  int          mb_nr = img->current_mb_nr;
  ...
  //上方一共1个像素点(实际上用了上方16个像素点，其他15个的像素点的信息均通过当前像素点改变坐标得到)
...
int dct_luma_16x16(int new_intra_mode)
{
  ...
  int ii,jj;
  int i1,j1;
  int M1[16][16];						//保存宏块16x16的预测误差
  int M4[4][4];							//从dct系数中，取出4x4的直流系数，进行hadamard变换，保存在M4中
  int M5[4],M6[4];
  int M0[4][4][4][4];					//保存16个4x4子块的预测误差，进行整数dct，保存在M0中
  ...
  // 3).M0变换后的系数中，抽取直流系数，保存于M4中，大小为4x4( pick out DC coeff )
  for (j=0;j<4;j++)
    for (i=0;i<4;i++)
      M4[i][j]= M0[0][i][0][j];

  // 4).M4中保存的直流系数，作hadamard变换, 仍然保存在M4中
  for (j=0;j<4;j++)
  {
    for (i=0;i<2;i++)
    {
  ...
  // 由于帧内16x16的AC系数就1个块，但是所用的cofAC是以4x4块为单位的，并且dct也是按4x4块为单位进行的
  // 所以这里将16x16的宏块，分为4x4个4x4块，对每个4x4块进行16个系数量化保存（实际上数组大小是17个，用没用到第17个，要看非零系数有没有16个）
  // 由于ACLevel中，保存的是非零系数，每个块的费零系数个数却是不一样的，为了指定非零系数结尾，所以要在最后1个非零系数的后面1个元素置0，表示结尾
  // 若16个系数全是非零系数，所以要在第17个元素上置0，因此数组长度实际申请的是17个
  for (jj=0;jj<4;jj++)
  {
    for (ii=0;ii<4;ii++)
    {
 ...
 *	  输入block_x, block_y在宏块内的4x4块的位置，像素单位
 * \par Output_
 *    nonzero: 0 if no levels are nonzero.  1 if there are nonzero levels.             \n
 *    coeff_cost: Counter for nonzero coefficients, used to discard expencive levels.
 *	  输出非零标志(nonzero)和非零系数的计数(coeff_cost)
 ************************************************************************
 */
int dct_luma(int block_x,int block_y,int *coeff_cost, int old_intra_mode)
{
  ...
  int*  ACLevel = img->cofAC[b8][b4][0];				//ACLevel
  int*  ACRun   = img->cofAC[b8][b4][1];				//ACRun
...
//*****************************************************************************************************************************************************
//2).Quant*********************************************************************************************************************************************
//Quant是dct的第二部分, 将dct量化值，保存于ACLevel/Run
  ...
  for (coeff_ctr=0;coeff_ctr < 16;coeff_ctr++)
  {
    ...
    {
		// 扫描到了第coeff_ctr个系数，返回该系数所在的坐标(因为采用的是zigzag扫描顺序，所以不是简单的i, j递增)方便指定系数值
        i=SNGL_SCAN[coeff_ctr][0];
        j=SNGL_SCAN[coeff_ctr][1];
    }
	...
	// |Zij| = (|Wij|*MF + f)>>qbits
	// sign(Zij) = sign(Wij)
	// 其中，f是改善恢复图像的视觉效果.由帧内帧间级qbits决定
	// Wij是系数. MF = PF/Qstep * (1<<qbits)
	// PF是变换矩阵Ef中的元素. Qstep由QP决定
	// quant_coef[qp][i][j]就是MF， qp_rem就是使用的QP%6.MF是一个已经定好了的3维矩阵
    // qbits = 15+floor(QP/6);
	level = (abs (img->m7[i][j]) * quant_coef[qp_rem][i][j] + qp_const) >> q_bits;
    
	// 若幅值不为0，则在AC_LEVEL中记录
    if (level != 0)
    {
      nonzero=TRUE;
      if (level > 1)
	  ...
	  //scan_pos才是实际用的记录位置
	  //因为当level==0时，并不记录，而run增1.当level！=0时，才进来记录赋给ACLevel，并将之前的零系数个数run赋给ACRun, 且run重置
	  //ACLevel可正可负
	  ...
	  //反量化和部分逆dct, dequant_coef是在逆dct中，使用的矩阵的反量化
      ilev=level*dequant_coef[qp_rem][i][j]<<qp_per;
    }
	//img->m7>0, 则ilev>0, 否则ilev<0.
    img->m7[i][j]=sign(ilev,img->m7[i][j]);
  }
  //在ACLevel的最后，赋值0，以表示ACLevel的结束（最后位置）
  // 由于ACLevel中，保存的是非零系数，每个块的费零系数个数却是不一样的，为了指定非零系数结尾，所以要在最后1个非零系数的后面1个元素置0，表示结尾
  // 若16个系数全是非零系数，所以要在第17个元素上置0，因此数组长度实际申请的是17个
  ACLevel[scan_pos] = 0;
...
//2).IDCT**********************************************************************************************************************************************
  ...
  // enc_picture->imgY保存解码（将预测误差作DCT->Q->IQ->IDCT，再加上预测值进行重建）的4x4子块像素
  for (j=0; j < BLOCK_SIZE; j++)
    for (i=0; i < BLOCK_SIZE; i++)
...
int dct_chroma(int uv,int cr_cbp)
{
  ...
  int   b4;
  // cofDC[0]是保存帧内16x16亮度块的直流系数的hadamard的
  // cofDC[1/2]是保存帧内8x8色度块的直流系数的hadamard的
  ...
  for (n2=0; n2 <= BLOCK_SIZE; n2 += BLOCK_SIZE)
  {
    ...
    {
      b4      = 2*(n2/4) + (n1/4);
	  //cofAC[0-3]都是保存16x16亮度块指定8x8块的预测误差系数的
	  //cofAC[4/5]都是保存8x8色度块的预测误差系数的
	  //b4进一步指定8x8块中的4x4块
