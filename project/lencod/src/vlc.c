 ...
 *	  写n个bit
 * \param n
 *    length in bits  bit长度
 * \param tracestring
 *    the string for the trace file		在trace文件中的字符串信息（TRACE功能开启时）
 * \param value
 *    the value to be coded				被编码的值
 *  \param part
 *    the Data Partition the value should be coded into	将值写入数据分区
 *
 * \return
 *    Number of bits used by the coded syntax element 
 *
