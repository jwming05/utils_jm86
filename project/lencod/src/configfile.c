...
static char *GetConfigFileContent (char *Filename);			//获得配置文件内容保存于数组缓冲
static void ParseContent (char *buf, int bufsize);			//解析内容，将缓存内容解析至结构体
static int ParameterNameToMapIndex (char *s);				//
static void PatchInp ();									//检查输入参数
static void ProfileCheck();
static void LevelCheck();


#define MAX_ITEMS_TO_PARSE  10000							//最大解析项数量
...
void Configure(int ac, char *av[])
{
  ...
  ParseContent (content, strlen(content));				//将缓冲区中的内容赋予configinput
  printf ("\n");
  free (content);
...
char *GetConfigFileContent (char *Filename)
{
  ...
  FileSize = fread (buf, 1, FileSize, f);			//将内容全部读取
  buf[FileSize] = '\0';
...
static void PatchInp ()
{
  ...
  //检查大小信息是否正确.(宽高需要是16的倍数，这样才适合16x16宏块分块)
