...
void code_a_picture(Picture *pic)
{
  ...
  img->currentPicture = pic;		//pic和img->currentPicture相连。对img->currentPicture的操作，就是对pic/frame/frame_pic的操作
  ...
  pic->no_slices = 0;				//
  pic->distortion_u = pic->distortion_v = pic->distortion_y = 0.0;		//对yuv的扭曲系数

  // restrict list 1 size
  // 参考帧列表list的大小
  // 当当前帧为非B帧时，list1的大小为0
  ...
  init_lists(img->type, img->structure);		//生成参考帧列表listX[0][PicNum], listX[1][PicNum]
  ...
  if (img->structure==FRAME)
    init_mbaff_lists();					//宏块级帧场自适应时，由于场编码会用到参考场，所以该函数是通过参考帧设置参考场的
  ...
  FmoInit(img, active_pps, active_sps);			//初始化fmo(多片组)相关信息, 主要是FirstMBInSlice、mapUnitToSliceGroupMap、MBAmap
	
 //! picture level initialization of FMO
  FmoStartPicture ();							 //用来设置每个片组的首个宏块地址

  /*
  一帧图像的编码流程：
  一帧图像分了多个片组，每个片组由很多片组成，每个片由很多宏块组成。
  当编码完一个片组，判断编码的宏块是否=总宏块数，若相等，则一帧图像编码完成，否则编码新的片组。
  对一个片组的编码，是分别对每个片进行编码，每编码一个片，获取下一个片的宏块起始位置，保存在FirstMBInSlice中。
  通过FmoSetLastMacroblockInSlice，设置FirstMBInSlice[SliceGropu]，即下一个片的起始地址(也就是上一片中的最后一个块的地址)。
  通过FmoSliceGroupCompletelyCoded，来判断当前片组是否编码完成，完成的标致是已经编码了片组中的最后一个片，FirstMBInSlice[SliceGropu]=-1;
  */
  while (NumberOfCodedMBs < img->total_number_mb)       // loop over slices
  {
    // Encode one SLice Group
    while (!FmoSliceGroupCompletelyCoded (SliceGroup))	//判断当前片组是否编码完成
    {
      // Encode the current slice
//编码一个片组(片组序号，当前操作的图像)***************************************************************************************************************
      NumberOfCodedMBs += encode_one_slice (SliceGroup, pic);		//编码一个片，并且返回已经编码的宏块数
//*****************************************************************************************************************************************************

	  FmoSetLastMacroblockInSlice (img->current_mb_nr);				//img->current_mb_nr是当前处理宏块的地址[ 0 — (total_number_mb-1) ]
      // Proceed to next slice
      img->current_slice_nr++;
      stat->bit_slice = 0;
    }
    // Proceed to next SliceGroup
    SliceGroup++;													//当前帧中的下一个片组进行编码。
  }
  FmoEndPicture ();
...
*/
  ...
  init_frame ();
  FrameNumberInFile = CalculateFrameNumber();					//计算该图像在文件中的帧数

  srcframe = AllocSourceframe (img->width, img->height);		//为即将打开的图像，分配缓存
  ReadOneFrame (FrameNumberInFile, input->infile_header, img->width, img->height, srcframe);		//读取一帧至srcframe
  CopyFrameToOldImgOrgVariables (srcframe);						//将srcframe中的数据，拷贝到imgY_org_frm[x][y]和imgUV_org_frm[uv][x][y]中
...
//********************************************************************************************************************************************
  ...
  enc_picture=enc_frame_picture;					//enc_picture和enc_frame_picture联立
  stat->em_prev_bits_frm = 0;
  stat->em_prev_bits = &stat->em_prev_bits_frm;

  if (img->MbaffFrameFlag)
  {
    CopyTopFieldToOldImgOrgVariables (srcframe);	//将srcframe中的顶场像素，拷贝至imgUV_org_top[uv][x][y]
    CopyBottomFieldToOldImgOrgVariables (srcframe);	//将srcframe中的底场像素，拷贝至imgUV_org_bottom[uv][x][y]
  }

  img->fld_flag = 0;

//编码*************************************************************************************************************************************************
  code_a_picture(frame);				//frame就是frame_pic,和img->currentPicture相连
//*****************************************************************************************************************************************************
 ...
 *    将sf中的帧像素，拷贝到old-style变量 imgY_org_frm 和 imgUV_org_frm中
 * \param sf
...
static Sourceframe *AllocSourceframe (int xs, int ys)
{
  ...
  if ((sf = calloc (1, sizeof (Sourceframe))) == NULL) no_mem_exit ("ReadOneFrame: sf");						//为sf分配空间
  
  //yf,为一帧中的亮度y分配空间;  yt,为一顶场中的亮度y分配空间;   yb,为一底场中的亮度y分配空间;
...
static void ReadOneFrame (int FrameNoInFile, int HeaderSize, int xs, int ys, Sourceframe *sf)
{
  ...
  const unsigned int bytes_y = xs*ys;						//bytes_y是亮度的字节数
  const unsigned int bytes_uv = (xs*ys)/4;					//bytes_uv是色度的字节数（单个色度）
  const int framesize_in_bytes = bytes_y + 2*bytes_uv;		//framesize_in_bytes是跳过一帧需要的字节数
  ...
  // 该循环是为了支持文件字节数大于int型最大值时，fseek指向带来的麻烦。很有可能fseek在源文件中跳过的字节数，大于int型最大值。

  //input->start_frame是在配置文件中，指定从原文件哪帧开始进行编码
  ...
  // read it.
  // 从这里开始，文件指针已经指向了我们欲读取的文件位置，现在可以进行读取了。

  if (fread (sf->yf, 1, bytes_y, p_in) != bytes_y)		//从p_in中，读取bytes_y个数据，bytes_y是当前图像亮度的字节数
  {
    printf ("ReadOneFrame: cannot read %d bytes from input file, unexpected EOF?, exiting", bytes_y);
    report_stats_on_error();
    exit (-1);
  }
  if (fread (sf->uf, 1, bytes_uv, p_in) != bytes_uv)	//从p_in中，读取bytes_uv个数据，bytes_uv是当前图像色度的字节数
  {
    printf ("ReadOneFrame: cannot read %d bytes from input file, unexpected EOF?, exiting", bytes_uv);
    report_stats_on_error();
    exit (-1);
  }	
  if (fread (sf->vf, 1, bytes_uv, p_in) != bytes_uv)	//从p_in中，读取bytes_uv个数据，bytes_uv是当前图像色度的字节数
  {
    printf ("ReadOneFrame: cannot read %d bytes from input file, unexpected EOF?, exiting", bytes_uv);
    report_stats_on_error();
    exit (-1);
