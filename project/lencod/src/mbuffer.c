...
void init_dpb()
{
  ...
  dpb.size      = getDpbSize();			//获得解码图像缓存大小
...
//  dpb.size      = input->num_reference_frames + inp->successive_Bframe;
  ...
  dpb.used_size = 0;					//dpb已经使用的大小
  dpb.last_picture = NULL;				//dpb最后一张图像指针

  dpb.ref_frames_in_buffer = 0;			//缓冲区中短期参考帧的数量
  dpb.ltref_frames_in_buffer = 0;		//缓冲区中长期参考帧的数量

  dpb.fs = calloc(dpb.size, sizeof (FrameStore*));		//dpb的fs包含了所有重建帧
  if (NULL==dpb.fs) 
    no_mem_exit("init_dpb: dpb->fs");

  dpb.fs_ref = calloc(dpb.size, sizeof (FrameStore*));	//fs_ref包含了所有短期参考帧
  if (NULL==dpb.fs_ref) 
    no_mem_exit("init_dpb: dpb->fs_ref");

  dpb.fs_ltref = calloc(dpb.size, sizeof (FrameStore*));//fs_ltref包含了所有长期参考帧dpb.fs[i].frame.PicNum(.imgY, .order_num)
  if (NULL==dpb.fs_ltref) 
    no_mem_exit("init_dpb: dpb->fs_ltref");
  
  for (i=0; i<dpb.size; i++)							//对所有数组进行初始化操作
  {
 ...
 *	  这里面，将list初始化，具体的操作有：
 *	  1.计算所有dpb中所有参考的PicNum(根据参考帧的frame_num)
 *	  2.将list[0/1][list_idx] = dpb.fs_ref[list_idx]->frame 相连接, 因此list中指向了dpb中对应的参考帧信息(包括像素信息, PicNum, order_num)
 *	  3.对list[0]短期参考帧和长期参考帧根据对应的PicNum进行排序
 *    4.对list[1]短期参考帧和长期参考帧根据对应的pic_order_cnt进行排序
 ************************************************************************
 */
void init_lists(int currSliceType, PictureStructure currPicStructure)
{
  ...
  if (currPicStructure == FRAME)							//当前图片编码格式，帧编码
  {
	  //遍历所有的短期参考帧，设置每个参考的pic_num
    for (i=0; i<dpb.ref_frames_in_buffer; i++)				//在缓冲区中的参考帧数
    {
      if (dpb.fs_ref[i]->is_used==3)						//is_used==3时，用的是 场对 或者 帧 编码
      {
		  //计算帧编码时，每个帧的pic_num
		  //已用作参考帧，并且非长期参考帧时，计算对应的frame_num_wrap、PicNum
        if ((dpb.fs_ref[i]->frame->used_for_reference)&&(!dpb.fs_ref[i]->frame->is_long_term))
        {
			// 计算参考帧的frame_num_wrap
  ...
  //若当前片是I片 或 SI片，则不适用参考帧，参考帧对应的大小为0
  ...
  //若当前片是P片 或 SP片，则使用list0，不使用list1
  if ((currSliceType == P_SLICE)||(currSliceType == SP_SLICE))
  {
    ...
    if (currPicStructure == FRAME)  
    {
      ...
      for (i=0; i<dpb.ref_frames_in_buffer; i++)
      {
        ...
        {
			//现在参考帧进入list0 主要是 看used_for_reference是否为1，是否为短期参考帧（长期参考帧要等参考帧进完以后再进）
          if ((dpb.fs_ref[i]->frame->used_for_reference)&&(!dpb.fs_ref[i]->frame->is_long_term))
          {
...
//      printf("listX[0] (PicNum): "); for (i=0; i<list0idx; i++){printf ("%d  ", listX[0][i]->pic_num);} printf("\n");
	  ...
	  // 根据LongTermPicNum进行长期参考帧的排序
	  // 根据compare_pic_by_lt_pic_num_asc, 从listXSize[0]开始，在list0中，进行list0idx-listXsize[0]个数据的排序.
	  // listXSize[0]现在还只是短期参考帧的大小.
 ...
 *	  宏块级帧场自适应时，需要用到.因为如果用到场时，需要用到参考场，该函数就是通过参考帧设置参考场的
 ************************************************************************
 */
...
ColocatedParams* alloc_colocated(int size_x, int size_y, int mb_adaptive_frame_field_flag)
{
  ...
  s = calloc(1, sizeof(ColocatedParams));					//分配内存
  s->size_x = size_x;										//图像宽度
  s->size_y = size_y;										//图像高度


  get_mem3Dint (&(s->ref_idx), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE);			//生成3维矩阵: s->idx，
  get_mem3Dint64 (&(s->ref_pic_id), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE);		//生成3维矩阵: s->ref_pic_id,
  get_mem4Dint (&(s->mv), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE,2 );				//生成4维矩阵: s->mv

  get_mem2D (&(s->moving_block), size_x / BLOCK_SIZE, size_y / BLOCK_SIZE);				//生成2维矩阵: s->moving_block
  get_mem2D (&(s->field_frame), size_x / BLOCK_SIZE, size_y / BLOCK_SIZE);				//生成2维矩阵: s->filed_frame

  // 宏块帧场自适应标致
  if (mb_adaptive_frame_field_flag)
  {
    get_mem3Dint (&(s->top_ref_idx), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2);			//生成3维矩阵: s->top_ref_idx
    get_mem3Dint64 (&(s->top_ref_pic_id), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2);		//生成3维矩阵: s->top_ref_pic_id
    get_mem4Dint (&(s->top_mv), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2,2 );				//生成4维矩阵: s->top_mv
    get_mem2D (&(s->top_moving_block), size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2);				//生成2维矩阵: s->top_moving_block
    
    get_mem3Dint (&(s->bottom_ref_idx), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2);			//生成3维矩阵: s->bottom_ref_idx
    get_mem3Dint64 (&(s->bottom_ref_pic_id), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2);	//生成3维矩阵s->bottom_ref_pic_id
    get_mem4Dint (&(s->bottom_mv), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2,2 );			//生成4维矩阵: s->bottom_mv
    get_mem2D (&(s->bottom_moving_block), size_x / BLOCK_SIZE, size_y / BLOCK_SIZE/2);			//生成2维矩阵: s->bottom_moving_block
  }

  //s->mb_field = calloc (img->PicSizeInMbs, sizeof(int));

  s->mb_adaptive_frame_field_flag  = mb_adaptive_frame_field_flag;								//宏块帧场自适应标致
