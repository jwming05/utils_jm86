 ...
 *	  该函数主要初始化 mapUnitToSliceGroupMap,和MBAmap
 * \param img
 *    Image Parameter to be used for map generation
 * \param pps
 *    Picture Parameter set to be used for map generation
...
int FmoInit(ImageParameters * img, pic_parameter_set_rbsp_t * pps, seq_parameter_set_rbsp_t * sps)
{
  ...
  int k;
  for (k=0;k<MAXSLICEGROUPIDS;k++)				//MAXSLICEGROUPIDS = 8, 片组的最大数量
    FirstMBInSlice[k] = -1;						//FirstMBInSlice[k],	指第k个片组的首个宏块地址
  
  
  
  FmoGenerateMapUnitToSliceGroupMap(img, pps);	//生成mapUnitToSliceGroup（映射单元到片组的映射表）
  FmoGenerateMBAmap(img, sps);					//生成MBAmap（宏块到片组的映射表）
 ...
 *	  在每个新的图片开始，初始化FMO, 用来设置每个片组的首个宏块地址
 * \par Input:
 *    None
 ************************************************************************
 */
int FmoStartPicture ()
{
  ...
  for (i=0; i<MAXSLICEGROUPIDS; i++)
    FirstMBInSlice[i] = FmoGetFirstMBOfSliceGroup (i);	//计算第i个片组的首个宏块地址
  return 0;
}
 ...
 *    获得FMO中的下一个宏块地址。如果是返回的-1，则该片组结束
 * \par Input:
 *    CurrentMbNr
 ************************************************************************
 */
int FmoGetNextMBNr (int CurrentMbNr)
{
  ...
  //宏块地址未超出返回时，该宏块所处片组的ID != 当前片组时，循环
  //遍历了所有宏块，都未找到当前片组的宏块，则片组已经结束，退出。
  //在地址超出前，找到了属于当前片组的下一个宏块，则返回该宏块地址。
...
{
  int i = 0;
  //第i个宏块没有超出总宏块的情况下，若当前宏块所处的片组!=指定片组，则查看下一个宏块
  while ( (i<(int)img->PicSizeInMbs) && (FmoMB2SliceGroup (i) != SliceGroupID) )
    i++;

  //遍历完所有宏块，都无法和所指定的片组对上号，就说明这样的片组不存在
