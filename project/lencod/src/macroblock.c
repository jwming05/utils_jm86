...
void set_MB_parameters (int mb_addr)
{
  ...
  get_mb_block_pos(mb_addr, &img->mb_x, &img->mb_y);			//坐标,宏块为单位:mb_x, mb_y
  
  img->block_x = img->mb_x << 2;								//坐标，块单位:block_x, block_y
  img->block_y = img->mb_y << 2;

  img->pix_x   = img->block_x << 2;								//坐标，像素单位:pix_x, pix_y
  img->pix_y   = img->block_y << 2;

  img->opix_x   = img->pix_x;

  // 设置当前操作的像素imgY_org imgUV_org, 这两个变量会根据当前帧场模式进行变量连接
  if (img->MbaffFrameFlag)
  {
	 // 场块时，设置imgY_org/imgUV_org为场像素
...
void start_macroblock(int mb_addr, int mb_field)
{
  int i,j,k,l;
  int use_bitstream_backing = (input->slice_mode == FIXED_RATE || input->slice_mode == CALLBACK);
  Macroblock *currMB = &img->mb_data[mb_addr];				//当前宏块和img->mb_data[mb_adrr]连接
  Slice *curr_slice = img->currentSlice;					//当前片和img->currentSlice连接
  ...
  set_MB_parameters (mb_addr);								//设置宏块参数,主要是指定宏块地址的 宏块坐标 和 像素变量连接(根据帧场编码模式)
                       ...
                       )
{
  ...
  int    coeff_cost = 0;
  int    mb_y       = (block8x8 / 2) << 3;		//在宏块中的位置，像素单位
  int    mb_x       = (block8x8 % 2) << 3;		//在宏块中的位置，像素单位
  ...
  {
    pic_pix_y = img->opix_y + block_y;			//pic_pix_y是子块在图像中位置，像素坐标.每次操作1个4x4子块,该值步进4

    for (bxx=0, block_x=mb_x; block_x<mb_x+8; bxx+=4, block_x+=4)
    {
      pic_pix_x = img->opix_x + block_x;		//pic_pix_x是子块在图像中的位置，像素坐标.每次操作1个4x4子块,该值步进4

      cbp_blk_mask = (block_x>>2) + block_y;

      //===== prediction of 4x4 block =====
	  //利用函数LumaPrediction4x4根据已经得到的mv来计算每一块的预测值,保存在img->mpr中,
	  //预测4x4块的像素
	  ...
	  //然后利用这个残差调用函数dct_luma进行DCT, 量化,反DCT, 反量化, 之后得到重建值保存在enc_picture->imgY中.
	  //保存在enc_picture->imgY的操作，在dct_luma中进行
 ...
 *	  帧间亮度块残差编码,将一个宏块分成4个8x8块进行编码，每个8x8还有分成4个4x4进行预测，编码，重构
 ************************************************************************
 */
void
LumaResidualCoding ()
{
  ...
  for (block8x8=0; block8x8<4; block8x8++)
  {
	...
	//从中读取当前的
	//预测方向：p_dir
	//前向预测模式：fw_mode(这个是指的什么分割模式)，当前分割模式下，当前8x8块的分割模式
	//参考帧：refframe，当前分割模式下，当前8x8的参考帧
	...
	//利用SetModesAndRefframe中提取出来的当前8x8块的参数，执行该函数
	//注意，虽然该函数中要将8x8分为4个4x4小块进行，但是LumaResidualCoding这个函数只有宏块级才能进来
	//所以4x4小块的属性和所属的8x8块属性是相同的
...
//                     (input->WeightedBiprediction && (img->type == B_SLICE)));
  ...
  //===== INTRA PREDICTION =====
  //根据img->mb_data[img->current_mb_nr].c_ipred_mode提前当前色度预测模式
  //并将预测值从mprr_c[uv][c_mode]中提取出来，保存于mpr中
  if (p_dir==-1)
  {
 ...
 *    对于帧内预测，先提取mprr_c[uv][mode]中的像素到mpr中，
 *	  并计算预测误差，保存在img->m7
 *    再计算预测误差系数的变换系数保存在img->cofAC/cofDC中，以及重建值保存在enc_picture->imgUV和cbp
 ************************************************************************
 */
void ChromaResidualCoding (int* cr_cbp)
{
  ...
  for (*cr_cbp=0, uv=0; uv<2; uv++)
  {
	...
	//block_x, block_y是4x4块在宏块中的位置，block8是第几个4x4块
	//该处进行宏块色度像素的预测
	/************************************************************************************
		对于帧内预测模式:
		根据img->mb_data[img->current_mb_nr].c_ipred_mode提前当前色度预测模式
	...
	*************************************************************************************/
	...
	// 计算当前色度预测模式下的，色度预测误差, 保存于img->m7中

    if (img->NoResidueDirect)				//无预测误差模式, 预测值即重建值(重建值和预测值完全相等)
    {
      for (j=0; j<8; j++)
        for (i=0; i<8; i++)
        {
      ...
      if (skipped && img->type==SP_SLICE)	//skip模式，预测误差为0, 用预测值做重建值(重建值不等于预测值，但是忽略预测误差)
      {
        for (j=0; j<8; j++)
          for (i=0; i<8; i++)
          {
        ...
        if (skipped)						//skip模式，预测误差为0, 用预测值做重建值(重建值不等于预测值，但是忽略预测误差)
        {
          for (j=0; j<8; j++)
            for (i=0; i<8; i++)
            {
    ...
    // 得到当前色度预测模式的 预测误差变换系数，色度像素重建值(enc_picture->imgUV)，以及cbp
...
void IntraChromaPrediction8x8 (int *mb_up, int *mb_left, int*mb_up_left)
{
  ...
  mb_available_up                             = up.available;				//上方块是否可用
  mb_available_up_left                        = left[0].available;			//左上方块是否可用
...
int writeCoeff4x4_CAVLC (int block_type, int b8, int b4, int param)
{
  ...
  //***********************************************************************************************
  //event
  if (dptype == SE_LUM_AC_INTER)
  {
	  Event_WriteCoeff4x4_CAVLC(block_type, pLevel, pRun);
  }
...
int find_sad_16x16(int *intra_mode)
{
  ...
  int M0[4][4][4][4];		//将M1分成16个4x4子块，并且后对每个4x4小块进行hadamard变换，并保存在M0中
  int M3[4];				//hadamard变换中间变量
  int M4[4][4];				//保存dc系数的hadamard变换的结果
...
//**************************************************************************************************************************************************
    else
    {
	  //计算预测误差
		  for (j=0;j<16;j++)
		  {
			for (i=0;i<16;i++)
			{
			  M1[i][j]=imgY_org[img->opix_y+j][img->opix_x+i]-img->mprr_2[k][j][i];		//org - mprr_2[k], 第k模式的预测误差
			  M0[i%4][i/4][j%4][j/4]=M1[i][j];											//将M1分成了16个4x4小块存储, 为了方便hadamard变换
			  //M0[i][block_i][j][block_j]	i, j是4x4块行列坐标, block_x, block_y是4x4小块在宏块内的行列坐标
			}
		  }
	...
	// 最终计算current_intra_sad_2******************************************************************************************************************
	  ...
	  for (jj=0;jj<4;jj++)
      {
        for (ii=0;ii<4;ii++)
        {
		  //HxAx(H^T), 下面两个循环得到1个4x4块的hadamard系数
          ...
          for (i=0;i<4;i++)
          {
            ...
            for (j=0;j<4;j++)
              if ((i+j)!=0)			//排开直流系数，只将交流系数累计，作为sad
	...
	// Hadamard of DC koeff*************************************************************************************************************************
	  ...
	  //提取M0的直流系数到M4中，以便对直流系数矩阵做hadamard变换
      for (j=0;j<4;j++)
        for (i=0;i<4;i++)
          M4[i][j]=M0[0][i][0][j]/4;
        ...
        for (i=0;i<4;i++)
        {
          ...
          for (j=0;j<4;j++)
            current_intra_sad_2 += abs(M4[i][j]);		//DC系数变换取绝对值求和作为代价
        }
	// 得到最佳帧内预测模式**************************************************************************************************************************
        
		//记录最佳帧内16x16预测模式，并记录对应的代价
