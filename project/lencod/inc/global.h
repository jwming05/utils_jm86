...
typedef struct macroblock
{
  ...
  int                 cbp ;			//cbp（code_block_patern） 指定一个宏块6个8x8残差的编码方案
  ...
  int                 c_ipred_mode;      //!< chroma intra prediction mode, 色度帧类预测模式
...
typedef struct
{
  int             byte_pos;           //!< current position in bitstream;				比特流中的当前位置
  int             bits_to_go;         //!< current bitcounter							bit计数器
  ...
  byte            *streamBuffer;      //!< actual buffer for written bytes				被写入字节的实际缓冲
...
typedef struct
{
  ...
  char *yf, *uf, *vf;                    //!< frame representation	yuv_frame
  char *yt, *ut, *vt;                    //!< top field				yuv_top
  char *yb, *ub, *vb;                    //!< bottom field			yuv_bottom
} Sourceframe;

// global picture format dependend buffers, mem allocation in image.c
// 根据当前的帧场编码模式，将imgY_org和imgUV_org和对应的帧场变量(imgY/UV_org_top/bot)连接
...
typedef struct
{
  ...
  int pn;                      //!< picture number							图片号码
  int nb_references;		   //
  int current_mb_nr;		   //
  int total_number_mb;		   //宏块总数
  int current_slice_nr;
  int type;					   //当前图像类型(I P SP SI B)
  int structure;               //!< picture structure						??
  int num_reference_frames;    //!< number of reference frames to be used	被用到的参考帧数
  int max_num_references;      //!< maximum number of reference pictures that may occur	最大的参考帧数
  int qp;                      //!< quant for the current frame				当前帧的量化系数
  ...
  int subblock_x;              //!< current subblock horizontal	当前子块水平坐标
  int subblock_y;              //!< current subblock vertical	当前子块垂直坐标
  int is_intra_block;		   //是否为帧内块
  int is_v_block;
  int mb_y_upd;
  ...
  int mpr[16][16];             //!< current best prediction mode								当前最好预测模式的预测值
  int m7[16][16];              //!< the diff pixel values between orginal image and prediction	原始图像和预测图像的残差

  // 当前宏块中，指定8x8块，指定4x4块，的系数。(level/run是游程编码的信息，level是幅值，run是对应幅值的前0数，这个前0数是两个非0之间的0的数目)
  ...
  int****** all_mv;       //!< replaces local all_mv	all_mv保存了当前宏块各个4x4子块的运动矢量和参考帧
  int LFDisableIdc;
  int LFAlphaC0Offset;
  int LFBetaOffset;
...
typedef struct
{
  ...
  float bitr;                   //!< bit rate for current frame, used only for output til terminal	当前帧码率
  float bitr0;                  //!< stored bit rate for the first frame							存储第一帧码率
  float bitrate;                //!< average bit rate for the sequence except first frame
  int   bit_ctr;                //!< counter for bit usage											已用的bit统计
  int   bit_ctr_0;              //!< stored bit use for the first frame								第一帧已用的bit统计
  int   bit_ctr_n;              //!< bit usage for the current frame								当前帧已用的bit统计
  int   bit_slice;              //!< number of bits in current slice								当前片中的bit数
...
#define IMG_NUMBER (img->number-start_frame_no_in_this_IGOP)		//当前的帧的帧数-当前帧所在IP组的开始帧帧数=所在IP组中的帧数（一切除开B帧）
