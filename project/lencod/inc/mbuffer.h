...
typedef struct storable_picture
{
  ...
  byte **     imgY_ups;      //!< Y picture component upsampled (Quarter pel)	升采样的图像，是1/4级像素的图像
...
typedef struct frame_store
{
  ...
  unsigned  frame_num;				//参考帧的frame_num
  int       frame_num_wrap;			//参考帧的frame_num_wrap
  int       long_term_frame_idx;	//长期参考帧序号
  int       is_output;
  int       poc;					//参考帧的poc

  StorablePicture *frame;			//记录该参考帧的的图像信息
  StorablePicture *top_field;		//记录该参考帧的顶场图像信息
  StorablePicture *bottom_field;	//记录该参考帧的底场图像信息
