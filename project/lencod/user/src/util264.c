...
void ZigZagScan(int b8, int b4, int cofmatrix[4][4], int **cofAC)
{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			oldmat[i][j] = enc_picture->imgY[img->pix_y + block_y + j][img->pix_x + block_x + i];
		}
	}
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			newmat[i][j] = enc_picture->imgY[img->pix_y + block_y + j][img->pix_x + block_x + i];
		}
	}
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			difmat[i][j] = newmat[i][j] - oldmat[i][j];
		}
	}
}
